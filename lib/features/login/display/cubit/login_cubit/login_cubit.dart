...
class LoginCubit extends Cubit<LoginState> {
  ...
  void emailChanged({required String email}) {
    emit(UpdateLogin(
        isEmailValid: LoginValidation.isEmailValid(email),
  ...
  void passwordChanged({required String password}) {
    emit(UpdateLogin(
        ...
        isPasswordValid: LoginValidation.isPasswordValid(password)));
  }

  Future<void> loginWithCredentialsPressed(
      {required String email, required String password}) async {
    LoginRepositoryImpl repository =
        LoginRepositoryImpl(dataSource: LoginFirebaseDataSourceImpl());
    emit(const LoadingLogin());
    final failureOrLogin = await LoginWithEmailAndPassword(repository)
        .call(email: email, password: password);
    failureOrLogin.fold(
        (failure) => emit(FailLogin(error: failure.errorMessage!)),
